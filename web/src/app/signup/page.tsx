...
export default function SignupPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await apiPost<AuthResponse>("/auth/signup", { name: name.trim(), email, password });
      setToken(res.token);
      router.replace("/dashboard");
    } catch (err) {
      setError(err instanceof Error ? err.message : typeof err === "string" ? err : "Signup failed");
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <AuthCard>
      ...
      {submitting ? (
        <div className="mt-4 overflow-hidden rounded-lg">
          <IndeterminateProgressBar slim label="Creating account" />
        </div>
      ) : null}
